(async () => {
  ...
  function changeProductToCheck() {
    activeOptions.productToCheck = document.getElementById('AMDProductSelection').value;
    if (!isIncognito) {
      chrome.storage.local.set({
        productToCheck: activeOptions.productToCheck,
      }, () => {
      });
    }
  }
  ...
  function changeMinDelay() {
    activeOptions.minDelay = document.getElementById('minDelay').value;
    if (!isIncognito) {
      chrome.storage.local.set({
        minDelay: activeOptions.minDelay,
      }, () => {
      });
    }
  }
  ...
  function restoreOptions() {
    chrome.storage.local.get(defaultOptions, (items) => {
      ...
      activeOptions.productToCheck = items.productToCheck;
      activeOptions.minDelay = items.minDelay;
      if (!isIncognito) {
  ...
  document.addEventListener('DOMContentLoaded', async () => {
    chrome.runtime.sendMessage({ command: 'tabDetection' }, (response) => {
      isIncognito = !!(response.isIncognito);
  ...
  !isIncognito ? `<div>
              ...
              </div>` : 'Configure additional settings in a non-incognito tab.'
  }`;
